...
import appbase.gateway.smtp.attachment;
...
struct SmtpMessage {
	...
	private string attachmentsToString() const {
		...
		return (result.length ? result[0..$ - 2] : "") ~ "\r\n";
	}
	...
	string toString() const {
		...
		if (!attachments.length) {
			return format(tFrom, sender.name, sender.address)
				...
				~ cc()
				~ format(tSubject, buildBase64Msg(subject))
				~ mime
				~ tMultipart
				~ format(tReplyTo, replyTo) ~ crlf
				~ "--" ~ boundary ~ crlf
				~ messageWithAttachments() ~ crlf;
		} else {
			return format(tFrom, sender.name, sender.address)
				~ format(tTo, recipients[0].name, recipients[0].address)
				~ cc()
	...
	private string buildBase64Msg(const string msg) const {
		return "=?UTF-8?B?" ~ cast(string) Base64.encode(cast(ubyte[]) msg) ~ "?=";
	}
}
